...
x_train, x_test = train_test_split(xy, test_size = 10/31, random_state = 0)
...
for i in range(s_train):
    ...
    # for j in [t-27, t-20, t-13, t-6, t, t+7]:
    for j in [t-28, t-27, t-20, t-13, t-6, t]:
...
import bp
# Applying back propagation model to training set
w,mse_train,y_pred_train = bp.bp_train(learning_rate, s_train, m_train, v_train, xx_train)

# Applying back propagation model to testing set
mse_test,y_pred_test = bp.bp_test(learning_rate, s_test, m_test, v_test, xx_test, w)
...
plt.figure()
plt.axis([0,s_test, -3*max(xx_test[:s_train,5]) , 3*max(xx_test[:s_train,5])])
